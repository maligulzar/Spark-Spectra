...
object Invoker {
  ...
  def invoked(id: Int, dataDir: String): Unit = {
   ...
   // if (!ids.contains(dataDir, id)) {
     ...
     // val writer1 =new FileWriter("/Users/malig/workspace/git/Spark-Spectra/temp.data", true)
      //writer1.append(dataDir + " - > " + id.toString + '\n').flush()
      ids.put((dataDir, id), ())
    //}
  }
  ...
  def invoked(files: Seq[File]): List[Int] = {
    var acc = List[Int]()
    files.foreach { file =>
